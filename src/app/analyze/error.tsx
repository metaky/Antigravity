...
export default function AnalyzeErrorPage({
  error,
  reset,
}: AnalyzeErrorPageProps) {
  useEffect(() => {
    analytics.captureException(error, {
      route: "/analyze",
      surface: "analyze_error_boundary",
      digest: error.digest,
    });
  }, [error]);
      ...
      <section className="mx-auto flex max-w-xl flex-col items-center gap-5 rounded-2xl border border-[var(--wc-ochre)]/25 bg-white p-8 text-center shadow-sm">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm leading-6 text-[var(--wc-brown-dark)]">
            ...
            issue. Try again, or return to the analyzer and upload the document
            once more.
          </p>
        </div>
        <div className="flex flex-wrap justify-center gap-3">
          ...
          <Button asChild variant="outline-organic">
            <Link href="/analyze">Back to analyzer</Link>
          </Button>
        </div>
      </section>
    </main>
