...
export function isAnalyzeFindingCategory(
  ...
  return typeof value === "string" && ANALYZE_FINDING_CATEGORIES.has(value as AnalyzeFindingCategory);
}

export function isAnalyzeFindingStatus(
  value: unknown,
): value is AnalyzeFindingStatus {
  return typeof value === "string" && ANALYZE_FINDING_STATUSES.has(value as AnalyzeFindingStatus);
}
